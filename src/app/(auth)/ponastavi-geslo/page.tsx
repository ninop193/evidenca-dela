...
export default function PonastaviGesloPage() {
  ...
  // Preveri, ali obstaja seja za ponastavitev (uporabnik je prišel prek povezave).
  useEffect(() => {
          ...
          {ready === "invalid" ? (
            <div className="text-center">
              ...
              <p className="mt-1.5 text-sm text-slate-500">
                Povezava za ponastavitev je potekla ali je bila že uporabljena.
              </p>
              <Link href="/pozabljeno-geslo" className="mt-6 inline-block text-sm font-semibold text-brand-600 hover:text-brand-700">
                Zahtevaj novo povezavo
              </Link>
            </div>
          ) : (
            <>
              <h1 className="text-xl font-bold text-slate-900">Nastavi novo geslo</h1>
              <p className="mt-1 text-sm text-slate-500">Vpiši novo geslo za svoj račun.</p>
              <form onSubmit={handleSubmit} className="mt-6 space-y-4">
                <label className="block">
                  <span className="mb-1.5 block text-sm font-medium text-slate-700">Novo geslo</span>
